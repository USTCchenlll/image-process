...
% % Input parameters
ztack_image_file_location = 'Z:\CL\20221003_jisui_no18\z_projection\';
position_file = 'D:\CL\jisui\jisui_image_analysis\20221003_img_flow\intermediate_data\2_stage_data\js18_position_1003.txt'; %only list of location
image_size = [2048,2048]; %[x,y]
overlap_percent = 0.1;
% get each slice fov
file_output_location = 'D:\CL\jisui\jisui_image_analysis\20221003_img_flow\intermediate_data\2_stage_data\';
file_name_qianzhui = '20221003_jisui18';
slice_cut_parameter = importdata([file_output_location 'all_slice_position1003.xlsx']);
...
for slice_i = 1 : slice_num 
    ...
    img_start = double(imread([ztack_image_file_location 'round1\max_' file_name_qianzhui '_round11_DAPI_s'  num2str(temp_slice_stage(4,1))   '.ome.tif'  ]));
	big_slice_image(coordinate_stage{4,1,slice_i}(1)+1:coordinate_stage{4,1,slice_i}(1)+image_size(2),coordinate_stage{4,1,slice_i}(2)+1:coordinate_stage{4,1,slice_i}(2)+image_size(2)) = img_start;
	for i1 = 2:length(temp_slice_stage(1,:))
        if temp_slice_stage(4,i1)>0
        img_next = double(imread([ztack_image_file_location 'round1\max_' file_name_qianzhui '_round11_DAPI_s'  num2str(temp_slice_stage(4,i1))   '.ome.tif'  ]));    
        ...
        big_slice_image(coordinate_stage{4,i1,slice_i}(1)+1:coordinate_stage{4,i1,slice_i}(1)+image_size(2),coordinate_stage{4,i1,slice_i}(2)+1:coordinate_stage{4,i1,slice_i}(2)+image_size(2)) = img_next;
        img_start = img_next;
        end
	...
	for i1 = 1:length(temp_slice_stage(1,:))
        ...
        img_start = double(imread([ztack_image_file_location 'round1\max_' file_name_qianzhui '_round11_DAPI_s'  num2str(temp_slice_stage(i2+1,i1))   '.ome.tif'  ]));
        img_next = double(imread([ztack_image_file_location 'round1\max_' file_name_qianzhui '_round11_DAPI_s'  num2str(temp_slice_stage(i2,i1))   '.ome.tif'  ]));
        ...
        big_slice_image(coordinate_stage{i2,i1,slice_i}(1)+1:coordinate_stage{i2,i1,slice_i}(1)+image_size(2),coordinate_stage{i2,i1,slice_i}(2)+1:coordinate_stage{i2,i1,slice_i}(2)+image_size(2)) = img_next;
        end
        end
    ...
    imwrite(uint16(big_slice_image),[ file_output_location 'slice_'  num2str(slice_i)  'stitch_dapi.tif' ])
 ...
 save([ file_output_location 'coordinate_stage1003.mat'], 'coordinate_stage')
